...
class integc_market(osv.osv):
    _name = 'integc.market'
    _description = 'Market'

    def _amount_line_tax(self, cr, uid, line, context=None):
        val = 0.0
        for c in self.pool.get('account.tax').compute_all(cr, uid, line.tax_id, line.price_unit * (1-(line.discount or 0.0)/100.0), line.product_uom_qty, line.product_id, line.market_id.partner_id)['taxes']:
            val += c.get('amount', 0.0)
        return val

    def _amount_avenant(self, cr, uid, ids, field_name, arg, context=None):
        res = {}
        for market in self.browse(cr, uid, ids, context=context):
            res[market.id] = {
                'amount_total_avenant': 0.0,
            }
            val = 0.0
            for contract in market.contract_ids:
                if contract.type != 'base':
                    val += contract.amount_total
            res[market.id]['amount_total_avenant'] = val
        return res
    ...
    def _amount_total_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        res = {}
        for market in self.browse(cr, uid, ids, context=context):
            val = market.amount_total + market.amount_total_avenant
            res[market.id] = {
                'amount_total_market': val
            }
        return res
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for market in self.browse(cr, uid, ids, context=context):
            ...
            val = val1 = 0.0
            cur = market.pricelist_id.currency_id
            for line in market.market_line:
                val1 += line.price_subtotal
                val += self._amount_line_tax(cr, uid, line, context=context)
            res[market.id]['amount_tax'] = cur_obj.round(cr, uid, cur, val)
            res[market.id]['amount_untaxed'] = cur_obj.round(cr, uid, cur, val1)
            res[market.id]['amount_total'] = res[market.id]['amount_untaxed'] + res[market.id]['amount_tax']
        return res

    def _get_market_from_contract(self, cr, uid, ids, context=None):
        result = {}
        for contract in self.pool.get('integc.market.contract').browse(cr, uid, ids, context=context):
            result[contract.market_id.id] = True
        logging.warning('_get_market_from_contract %s' % result.keys())
        return result.keys()

    def _get_market(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('integc.market.line').browse(cr, uid, ids, context=context):
            result[line.market_id.id] = True
        return result.keys()
    ...
    def _get_duration(self, cr, uid, ids, field_name, args, context=None):
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
            if record.date_start and record.date_end:
                res[record.id] = ((datetime.strptime(record.date_end,'%Y-%m-%d') - datetime.strptime(record.date_start, '%Y-%m-%d')).days)
            else:
                res[record.id] = 0
        return res

    def _set_duration(self, cr, uid, id, name, value, args=None, context=None):
        if value:
            this = self.browse(cr, uid, id, context=context)
            duration = ((datetime.strptime(this.date_end,'%Y-%m-%d') - datetime.strptime(this.date_start, '%Y-%m-%d')).days) if record.date_end and record.date_start else 0
            self.write(cr, uid, id, {'duration': duration})
            return duration

    def _has_contracts(self, cr, uid, ids, name, args, context=None):
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
            res[record.id] = False
    ...
    _columns = {
        ...
        'date_start': fields.date('Date Start', required=False, readonly=False, select=True),
        'date_end': fields.date('Date End', required=False, readonly=False, select=True),
        'duration': fields.function(_get_duration, fnct_inv=_set_duration, string='Delay', type='integer'),
        'create_date': fields.datetime('Creation Date', readonly=True, select=True),
        'date_open': fields.date('Date Open', readonly=True, select=True),
        'date_done': fields.date('Date Done', readonly=True, select=True),
        'date_cancel': fields.date('Date Cancel', readonly=True),
        ...
        'market_line': fields.one2many('integc.market.line', 'market_id', 'Market Lines'),
        'contract_id': fields.many2one('integc.market.contract', 'Base Contract', domain="[('type', '=', 'base')]"),
        'contract_ids': fields.one2many('integc.market.contract', 'market_id', 'Other Contracts'),
        'has_contracts': fields.function(_has_contracts, string='Has contracts', type='boolean'),
        'attachment_ids': fields.many2many("ir.attachment", "integc_market_attachment_rel",
        ...
        'amount_untaxed': fields.function(_amount_all, digits_compute=dp.get_precision('Account'), string='Untaxed Amount',
            store={
                ...
                'integc.market.line': (_get_market, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
        ...
        'amount_total': fields.function(_amount_all, digits_compute=dp.get_precision('Account'), string='Total Base',
            store={
                'integc.market': (lambda self, cr, uid, ids, c={}: ids, ['market_line'], 10),
                'integc.market.line': (_get_market, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
            multi='sums', help="The total amount."),
        'amount_total_avenant': fields.function(_amount_avenant, digits_compute=dp.get_precision('Account'), string='Total Avenant',
            store={
                'integc.market': (lambda self, cr, uid, ids, c={}: ids, ['market_line'], 10),
                'integc.market.contract': (_get_market_from_contract, ['contract_line', 'state'], 10),
            },
            multi='sums', help="The total amount avenant."),
        'amount_total_market': fields.function(_amount_total_all, digits_compute=dp.get_precision('Account'), string='Total amount market',
            store={
                'integc.market': (lambda self, cr, uid, ids, c={}: ids, ['market_line','state'], 10),
    ...
    def create(self, cr, uid, values, context=None):
        ...
        values['project_id'] = self.pool.get('project.project').create(cr, uid, vals, context=context)
        return super(integc_market, self).create(cr, uid, values, context=context)
    ...
    def create_avenant(self, cr, uid, ids, context=None):
        for record in self.browse(cr, uid, ids, context=context):
            vals = {
                'type': 'avenant',
                'market_id': record.id,
            ...
            market_contract_id = self.pool.get('integc.market.contract').create(cr, uid, vals, context=context)
            res = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'integc', 'view_integc_market_contract_form')
            return {
                'res_id': market_contract_id,
                'name': _('Avenant Contract'),
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'integc.market.contract',
...
class integc_market_line(osv.osv):
    ...
    def _amount_line(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            taxes = tax_obj.compute_all(cr, uid, line.tax_id, price, line.product_uom_qty, line.product_id, line.market_id.partner_id)
            cur = line.market_id.pricelist_id.currency_id
    ...
    _name = 'integc.market.line'
    _description = 'Market line'
    _columns = {
        'market_id': fields.many2one('integc.market', 'Market', required=True, ondelete='cascade', select=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'tax_id': fields.many2many('account.tax', 'integc_market_line_tax', 'market_line_id', 'tax_id', 'Taxes', readonly=True, states={'draft': [('readonly', False)]}),
        ...
        'company_id': fields.related('market_id', 'company_id', type='many2one', relation='res.company', string='Company', store=True, readonly=True),
    }
    _order = 'market_id desc, sequence, id'
    _defaults = {
    ...
    def copy_data(self, cr, uid, id, default=None, context=None):
        ...
        return super(integc_contract_line, self).copy_data(cr, uid, id, default, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(integc_market_line, self).unlink(cr, uid, ids, context=context)

integc_market_line()


class integc_market_contract(osv.osv):
    _name = "integc.market.contract"
    _description = "Market contract"
    ...
    def _amount_balance(self, cr, uid, ids, context=None):
        res = []
        invoice_ids = []
        cr.execute('SELECT rel.invoice_id FROM integc_market_contract_invoice_rel AS rel WHERE rel.contract_id = %s' % ids[0])
        res = cr.fetchall()
        for x in res:
    ...
    def _get_contract_from_invoice(self, cr, uid, ids, context=None):
        ...
        if len(ids) > 1 :
            values = tuple(ids)
            cond = 'in'
        cr.execute('SELECT rel.contract_id FROM integc_market_contract_invoice_rel as rel WHERE rel.invoice_id %s %s' % (cond,values))
    ...
    def _search_balance(self, cr, uid, obj, name, args, context):
        ...
        for cond in args:
            ...
            cr.execute("SELECT rel.contract_id FROM integc_market_contract AS c, account_invoice AS i, integc_market_contract_invoice_rel AS rel WHERE rel.contract_id = c.id and i.id = re.invoice_id and i.state not in ('draft', 'cancel') having sum(amount_total) %s %%s" % (cond[1]),(amount,))
            #cr.execute("select id from integc_market_contract group by id having balance %s %%s" % (cond[1]),(amount,))
    ...
    def _get_contract(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('integc.market.contract.line').browse(cr, uid, ids, context=context):
    ...
    _columns = {
        ...
        'date_cancel': fields.date('Date Cancel', readonly=True),
        'user_id': fields.many2one('res.users', 'User', select=True, track_visibility='onchange'),
        'partner_id': fields.related('market_id', 'partner_id', type='many2one', relation='res.partner', store=True, string='Customer'),
        ...
        'partner_shipping_id': fields.many2one('res.partner', 'Delivery Address', readonly=True, required=True, states={'draft': [('readonly', False)]}, help="Delivery address for current contract."),
        'pricelist_id': fields.related('market_id', 'pricelist_id', type='many2one', relation='product.pricelist', string='Pricelist', readonly=True, help="Pricelist for current contract."),
        'currency_id': fields.related('market_id', 'currency_id', type="many2one", relation="res.currency", string="Currency", required=True),
        'analytic_account_id': fields.related('market_id', 'analytic_account_id', type='many2one', relation='account.analytic.account', string='Analytic account', help="The analytic account related to a contract."),
        'contract_line': fields.one2many('integc.market.contract.line', 'contract_id', 'Contract Lines'),
        'invoice_ids': fields.many2many('account.invoice', 'integc_market_contract_invoice_rel', 'contract_id', 'invoice_id', 'Invoices', readonly=True, help="This is the list of invoices that have been generated for this contract. The same contract may have been invoiced in several times (by line for example)."),
        ...
        'amount_untaxed': fields.function(_amount_all, digits_compute=dp.get_precision('Account'), string='Untaxed Amount',
            store={
                'integc.market.contract': (lambda self, cr, uid, ids, c={}: ids, ['contract_line'], 10),
        ...
        'attachment_ids': fields.many2many("ir.attachment", "integc_market_contract_attachment_rel",
    ...
    def create_invoice(self, cr, uid, ids, grouped=False, states=None, date_invoice = False, context=None):
        ...
        obj_contract_line = self.pool.get('integc.market.contract.line')
        partner_currency = {}
        if context is None:
            context = {}
        ...
        for val in invoices.values():
            if grouped:
                ...
                for o, l in val:
                    invoice_ref += o.name + '|'
                    cr.execute('insert into integc_market_contract_invoice_rel (contract_id,invoice_id) values (%s,%s)', (o.id, res))
                ...
                for order, il in val:
                    ...
                    cr.execute('insert into integc_market_contract_invoice_rel (contract_id,invoice_id) values (%s,%s)', (order.id, res))
        mod_obj = self.pool.get('ir.model.data')
        act_obj = self.pool.get('ir.actions.act_window')
        ...
        if len(invoice_ids)>1:
            ...
            res_ = mod_obj.get_object_reference(cr, uid, 'account', 'invoice_form')
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        contract_line_obj = self.pool.get('integc.market.contract.line')
    ...
    def action_open(self, cr, uid, ids, context=None):
        ...
        for c in self.browse(cr, uid, ids):
            ...
            self.write(cr, uid, [c.id], {'state': 'open', 'date_open': fields.date.context_today(self, cr, uid, context=context)})
            self.pool.get('integc.market.contract.line').button_confirm(cr, uid, [x.id for x in c.contract_line])
        return True
    ...
    def get_contract_balance(self, cr, uid, invoice_id, context=None):
        cr.execute('SELECT rel.contract_id FROM integc_market_contract_invoice_rel AS rel WHERE rel.invoice_id = %s' % invoice_id)
        res = cr.fetchall()
        balance = 0.0
        contract_name = ''
        if res:
...
integc_market_contract()


class integc_market_contract_line(osv.osv):
    ...
    def _contract_lines_from_invoice(self, cr, uid, ids, context=None):
        ...
        cr.execute("""SELECT DISTINCT cl.id FROM integc_market_contract_invoice_rel rel JOIN
                                                  integc_market_contract_line cl ON (cl.contract_id = rel.contract_id)
                                    WHERE rel.invoice_id = ANY(%s)""", (list(ids),))
        return [i[0] for i in cr.fetchall()]

    _name = 'integc.market.contract.line'
    _description = 'Market contract line'
    _columns = {
        'contract_id': fields.many2one('integc.market.contract', 'Partner Contract', required=True, ondelete='cascade', select=True, readonly=True, states={'draft':[('readonly',False)]}),
        'name': fields.text('Description', required=True, readonly=True, states={'draft': [('readonly', False)]}),
        'sequence': fields.integer('Sequence', help="Gives the sequence order when displaying a list of contract lines."),
        'product_id': fields.many2one('product.product', 'Product', domain=[('sale_ok', '=', True)], change_default=True),
        'invoice_lines': fields.many2many('account.invoice.line', 'integc_market_contract_line_invoice_rel', 'contract_line_id', 'invoice_id', 'Invoice Lines', readonly=True),
        'invoiced': fields.function(_fnct_line_invoiced, string='Invoiced', type='boolean',
            store={
                'account.invoice': (_contract_lines_from_invoice, ['state'], 10),
                'integc.market.contract.line': (lambda self,cr,uid,ids,ctx=None: ids, ['invoice_lines'], 10)}),
        'price_unit': fields.float('Unit Price', required=True, digits_compute= dp.get_precision('Product Price'), readonly=True, states={'draft': [('readonly', False)]}),
        'type': fields.selection([('make_to_stock', 'from stock'), ('make_to_order', 'on order')], 'Procurement Method', required=True, readonly=True, states={'draft': [('readonly', False)]},
         help="From stock: When needed, the product is taken from the stock or we wait for replenishment.\nOn order: When needed, the product is purchased or produced."),
        'price_subtotal': fields.function(_amount_line, string='Subtotal', digits_compute= dp.get_precision('Account')),
        'tax_id': fields.many2many('account.tax', 'integc_market_contract_line_tax', 'contract_line_id', 'tax_id', 'Taxes', readonly=True, states={'draft': [('readonly', False)]}),
        'address_allotment_id': fields.many2one('res.partner', 'Allotment Partner',help="A partner to whom the particular product needs to be allotted."),
        'product_uom_qty': fields.float('Quantity', digits_compute= dp.get_precision('Product UoS'), required=True, readonly=True, states={'draft': [('readonly', False)]}),
        'product_uom': fields.many2one('product.uom', 'Unit of Measure ', required=True, readonly=True, states={'draft': [('readonly', False)]}),
        'product_uos_qty': fields.float('Quantity (UoS)' ,digits_compute= dp.get_precision('Product UoS'), readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def copy_data(self, cr, uid, id, default=None, context=None):
        if not default:
            default = {}
        default.update({'state': 'draft',  'invoice_lines': []})
        return super(integc_market_contract_line, self).copy_data(cr, uid, id, default, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.state not in ['draft', 'cancel']:
                raise osv.except_osv(_('Invalid Action!'), _('Cannot delete a contract line which is in state \'%s\'.') %(rec.state,))
        return super(integc_market_contract_line, self).unlink(cr, uid, ids, context=context)

integc_market_contract_line()
